...
pub enum StorageError {
    ...
    ValueTooLarge(usize),
    /// The key being inserted exceeds the maximum of 3GiB
    KeyTooLarge(usize),
    /// The combined key+value length exceeds the maximum
    KeyValuePairTooLarge { key_len: usize, value_len: usize },
    Io(io::Error),
    PreviousIo,
    DatabaseClosed,
...
impl From<StorageError> for Error {
    fn from(err: StorageError) -> Error {
        match err {
            ...
            StorageError::KeyTooLarge(x) => Error::KeyTooLarge(x),
